...
function Cart({ ToggleCart }) {
  ...
  return (
    <motion.div
      ...
      className="z-20 fixed top-0 right-0 w-2/3 sm:w-2/5 md:w-1/3 lg:w-1/4 h-screen bg-white px-4 shadow-black shadow-lg"
    >
      <div className="h-20 flex items-center justify-between">
        <div className="flex items-center font-bold text-lg gap-x-4">
          {total > 0 && (
            <Link to="checkout">
              <button
                className="text-white text-md px-2 py-1"
                onClick={() => ToggleCart()}
              >
                Checkout
              </button>
      ...
      <div className="overflow-y-scroll pb-[100px] h-full flex flex-col">
        {cartItem.map((item) => (
          <div
            key={`cart-${item.id}`}
            className="relative md:flex border-t-2 border-gray-300 py-4 lg:flex items-center gap-x-4"
          >
            <div>
            ...
            <div className="font-bold">
              ...
              <div className="flex">
                ...
                <AiOutlineMinusSquare
                  ...
                  className="cursor-pointer"
                  onClick={() => {
                    Decrement(item.id, item.price, item.name);
                  }}
                />
              </div>
              <BsTrash
